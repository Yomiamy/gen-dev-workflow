...
> ⚠️ **已知限制：MCP 呼叫無法逐次帶 `--print-timeout`。** 逾時只能靠 server 層環境變數統一設定，單次派發無法調整，且預設遠長於一般任務——實務上等同無有效的即時保護，卡住多半仍得人為中斷（細節見 gen-dev-workflow plugin 的 `references/mcp-delegation-discipline.md`）。派發前把任務拆到合理粒度，別丟一個會跑半小時的大任務進去。
