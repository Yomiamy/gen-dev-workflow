...
> ⚠️ **已知限制：MCP 呼叫無法逐次帶 `--print-timeout`**——逾時只能靠 server 層環境變數統一設定，單次派發無法調整，且預設遠長於一般任務（細節見 gen-dev-workflow plugin 的 `references/mcp-delegation-discipline.md`）。大型 diff 分析實務上等同無有效保護，diff 過大時先自行縮範圍再派發。
