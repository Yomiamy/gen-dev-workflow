...
> ⚠️ **`agentType` 不可省略**：省略時 agent 不會套用該角色 `agents/*.md`（plugin 內）的 frontmatter（implementer 的 `model: sonnet`、verifier 的 `model: opus`），等於放棄角色綁定。`opts.model` 只覆寫 model，不會補上 agentType。
>
