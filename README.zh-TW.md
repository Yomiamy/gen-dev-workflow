...
| **5** 回覆 Review | 逐條意見判斷 → reviewer 交叉驗證 → publisher 更新 PR | responder → reviewer → publisher | `sonnet`／`opus`／`sonnet` | `high`／`xhigh`／`high` | ❌ 不委派 |
...
暫停頻率用 `pause_level` 調：
- `strict`（預設，全停）
- `balanced`（只停計畫、實作完成、PR 前）
- `autonomous`（全不停，會讓 `gh pr create` 免確認執行，選前必須先警示）
